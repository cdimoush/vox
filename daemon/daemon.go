...
// Events returns a receive-only channel of transitions. Subscribers must
// drain the channel — slow consumers drop events (the channel is buffered
// but non-blocking on send).
...
// Shutdown closes the events channel. Call once, last.
...
// transitionLocked updates state and emits an event. mu must be held.
func (d *Daemon) transitionLocked(to State, text string, err error) {
	ev := Event{From: d.state, To: to, Text: text, Err: err}
	d.state = to
	// Non-blocking send — subscribers that fall behind lose events.
	select {
	case d.events <- ev:
	default:
	}
